...
```swift
AppConfig(useMock: false, baseURLString: "http://localhost:8080", projectId: "proj_artoo")
```
...
| Tasks (grouped)   | `GET /projects/:projectId/tasks`            |
...
- Bootstrap is `POST /api/v1/bootstrap` returning `{ organization, user, projects[], actor }`.
...
- Exact task **status** vocabulary and transitions (the client tolerates unknowns
  via `.other`, but the action mapping in `TaskDetailViewModel.availableActions`
  assumes `backlog/ready/assigned/in_progress/review/...`).
- `review` body shape (`{ decision: "accept" | "request_changes", comment? }`) and
  approvals `resolve` body (`{ decision: "approve" | "reject", comment? }`).
- Whether `assign` requires capabilities/model-profile beyond `assigneeType/assigneeId`.
