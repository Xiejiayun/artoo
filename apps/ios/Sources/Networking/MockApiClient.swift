...
public actor MockApiClient: ApiClientProtocol {
    ...
    public func assign(taskId: String, request: AssignRequest) async throws -> TaskResponse {
        ...
        let run = Run(
            ...
            taskId: taskId,
            agentInstanceId: request.assigneeId,
            status: .running
        )
        runs[run.id] = run
    ...
    public func review(taskId: String, request: ReviewRequest) async throws -> TaskResponse {
        let target: TaskStatus = request.decision == "accept" ? .accepted : .inProgress
...
public extension MockApiClient {
    static func demo() -> MockApiClient {
        ...
        let run = Run(
            ...
            status: .completed,
            summary: "Implemented the approvals list and resolve flow.",
        ...
        let snapshot = TaskSnapshot(
            task: inboxTask,
            room: Room(id: "room_1", taskId: "task_1", title: "Inbox work surface"),
            runs: [run],
            approvals: [approval],
            artifacts: [
                Artifact(
