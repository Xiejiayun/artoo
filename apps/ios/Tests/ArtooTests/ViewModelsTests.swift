...
final class ViewModelsTests: XCTestCase {
    ...
    func testTaskDetailLifecycleActionsFollowStatus() async {
        ...
        await model.assign(assigneeType: "agent", assigneeId: "agent_claude")
        XCTAssertEqual(model.state.value?.task.status, .assigned)
        XCTAssertEqual(model.availableActions, [.retry])
        XCTAssertEqual(model.state.value?.runs.isEmpty, false)
    }

    func testReviewAcceptMovesTaskToAccepted() async {
        ...
        XCTAssertEqual(model.state.value?.task.status, .accepted)
    }
}
