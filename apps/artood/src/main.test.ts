...
describe("loadConfigFromEnv", () => {
  ...
  it("throws naming the missing required var(s)", () => {
    expect(() => loadConfigFromEnv({ ARTOO_NODE_ID: "c1" })).toThrow(/ARTOO_NODE_URL/);
    expect(() => loadConfigFromEnv({ ARTOO_NODE_URL: "ws://x" })).toThrow(/ARTOO_NODE_ID/);
    expect(() => loadConfigFromEnv({})).toThrow(/ARTOO_NODE_URL.*ARTOO_NODE_ID/);
  });
});
