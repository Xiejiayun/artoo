 ...
 * - `ARTOO_ALLOWED_ROOTS`      `;`/`,`-separated filesystem roots the adapters may operate in
...
export function loadConfigFromEnv(env: NodeJS.ProcessEnv): ArtoodConfig {
  ...
  if (url === undefined || url === "" || nodeId === undefined || nodeId === "") {
  ...
  return {
    ...
    allowedRoots: splitList(env.ARTOO_ALLOWED_ROOTS),
