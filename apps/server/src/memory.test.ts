...
describe("memory propose/curate (#21 Phase B)", () => {
  ...
  it("requires the scope's binding ref for task/project/code memories", async () => {
    server = await buildTestServer();
    expect((await propose(server, { scope: "project", text: "project rule" })).statusCode).toBe(400);
    expect((await propose(server, { scope: "code", text: "code rule" })).statusCode).toBe(400);
    expect((await propose(server, { scope: "task", text: "task rule" })).statusCode).toBe(400);
    expect((await propose(server, { scope: "organization", text: "org rule" })).statusCode).toBe(201);
  });
  ...
  it("validates list filters", async () => {
    server = await buildTestServer();
    expect((await server.app.inject({ method: "GET", url: "/api/v1/memories?scope=global" })).statusCode).toBe(400);
    expect((await server.app.inject({ method: "GET", url: "/api/v1/memories?status=archived" })).statusCode).toBe(400);
  });
});
